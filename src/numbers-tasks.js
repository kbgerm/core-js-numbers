...
function getAverage(value1, value2) {
  const number = value1 / 2 + value2 / 2;
  return number;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const magnitute1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitute2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  const cosTheta = dotProduct / (magnitute1 * magnitute2);
